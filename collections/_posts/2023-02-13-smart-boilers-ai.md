...
First, we need to define our reward. Energy prices can be a good fit. Altough then, the reward of the agent should be the inverse of the energy cost. Because, the more energy you consume, the lower the score should be. This will then result in an energy efficient final control policy. Additionally, we can put the cost of self-produced sustainble energy, for example by means of a rooftop solar installation, to a very small number. As a result, the agent will able to learn that hot water can be made cheaply when the sun is shining.
